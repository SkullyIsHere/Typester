...
import { MonkeyResponse } from "../../utils/typester-response";
...
export async function getQuotes(
  ...
  if (quoteMod === true) {
    ...
    throw new TypesterError(403, "You are not allowed to view submitted quotes");
  }
  const data = await NewQuotesDAL.get(quoteModString);
  return new TypesterResponse("Quote submissions retrieved", data);
}
...
export async function addQuote(
  ...
  return new TypesterResponse("Quote submission added");
}
...
export async function refuseQuote(
  ...
  return new TypesterResponse("Quote refused");
}
...
export async function getRating(
  ...
  return new TypesterResponse("Rating retrieved", data);
}
...
export async function submitRating(
  ...
  return new TypesterResponse(responseMessage);
}
...
export async function reportQuote(
  ...
  const newReport: Typester.Report = {
    _id: new ObjectId(),
    id: uuidv4(),
    type: "quote",
  ...
  return new TypesterResponse("Quote reported");
}
