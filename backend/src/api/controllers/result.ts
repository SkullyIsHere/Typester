...
import TypesterStatusCodes from "../../constants/typester-status-codes";
import {
  incrementResult,
...
import { buildTypestermail } from "../../utils/monkey-mail";
...
export async function getResults(
  ...
  return new TypesterResponse("Results retrieved", results);
}
...
export async function getLastResult(
  ...
  return new TypesterResponse("Result retrieved", results);
}
...
export async function deleteAll(
  ...
  return new TypesterResponse("All results deleted");
}
...
export async function updateTags(
  ...
  return new TypesterResponse("Result tags updated", {
    tagPbs,
  });
}
...
export async function addResult(
  ...
  if (isTestTooShort(result)) {
    const status = TypesterStatusCode.TEST_TOO_SHORT;
    throw new TypesterError(status.code, status.message);
  }
  ...
  if (anticheatImplemented()) {
    if (!validateResult(result)) {
      const status = TypesterStatusCodes.RESULT_DATA_INVALID;
      throw new TypesterError(status.code, "Result data doesn't make sense");
    }
  } else {
    if (process.env.MODE !== "dev") {
  ...
  if (lastResultTimestamp && nowNoMilis < earliestPossible - 1000) {
    ...
    const status = TypesterStatusCodes.RESULT_SPACING_INVALID;
    throw new TypesterError(status.code, "Invalid result spacing");
  }
  ...
  if (
    ...
    if (!result.keySpacingStats || !result.keyDurationStats) {
      const status = TypesterStatusCodes.MISSING_KEY_DATA;
      throw new TypesterError(status.code, "Missing key data");
    }
    if (anticheatImplemented()) {
      if (!validateKeys(result, uid)) {
        ...
        if (autoBanConfig.enabled) {
          ...
          if (didUserGetBanned) {
            const mail = buildTypestermail({
              subject: "Banned",
              body: "You have been banned for triggering the anticheat system. If you believe this is a mistake, please contact support.",
            });
            UserDAL.addToInbox(uid, [mail], req.ctx.configuration.users.inbox);
          }
        }
        const status = TypesterStatusCodes.BOT_DETECTED;
        throw new TypesterError(status.code, "Possible bot detected");
      }
    } else {
      if (process.env.MODE !== "dev") {
        throw new Error("No anticheat module found");
  ...
  return new TypesterResponse("Result saved", data);
}
...
async function calculateXp(
  result,
  xpConfiguration: Typester.Configuration["users"]["xp"],
  uid: string,
