...
const PATH_TO_REPO = "../../../../typester-new-quotes";
...
export async function add(
  ...
  if (!git) throw new TypesterError(500, "Git not available.");
  const quote = {
    _id: new ObjectId(),
    text: text,
    source: source,
...
export async function get(language: string): Promise<Typester.NewQuote[]> {
  if (!git) throw new TypesterError(500, "Git not available.");
  const where: {
    approved: boolean;
    language?: string;
...
export async function approve(
  ...
  const targetQuote = await db
    .collection<Typester.NewQuote>("new-quotes")
  ...
  if (fs.existsSync(fileDir)) {
    ...
    quoteObject.quotes.every((old) => {
      if (stringSimilarity.compareTwoStrings(old.text, quote.text) > 0.8) {
        throw new TypesterError(409, "Duplicate quote");
      }
    });
    let maxid = 0;
