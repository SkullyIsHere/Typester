...
interface RankReturn extends Typester.LeaderboardEntry {
  count?: number;
}
...
export async function update(
  ...
  const lb = await db
    .collection<Typester.User>("users")
    .aggregate<Typester.LeaderboardEntry>(
      [
        {
          $match: {
            [str + ".wpm"]: {
