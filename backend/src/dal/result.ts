import _ from "lodash";
import { DeleteResult, ObjectId, UpdateResult } from "mongodb";
import TypesterError from "../utils/error";
import * as db from "../init/db";

import { getUser, getTags } from "./user";

type TypesterResult = MonkeyTypes.Result<Typester.Mode>;

export async function addResult(
  uid: string,
  result: TypesterResult
): Promise<{ insertedId: ObjectId }> {
  let user;
  try {
    user = await getUser(uid, "add result");
  } catch (e) {
    user = null;
  }
  if (!user) throw new TypesterError(404, "User not found", "add result");
  if (result.uid === undefined) result.uid = uid;
  // result.ir = true;
  const res = await db
    .collection<Typester>("results")
    .insertOne(result);
  return {
    insertedId: res.insertedId,
  };
}

export async function deleteAll(uid: string): Promise<DeleteResult> {
  return await db.collection<Typester>("results").deleteMany({ uid });
}

export async function updateTags(
  uid: string,
  resultId: string,
  tags: string[]
): Promise<UpdateResult> {
  const result = await db
    .collection<Typester>("results")
    .findOne({ _id: new ObjectId(resultId), uid });
  if (!result) throw new MonkeyError(404, "Result not found");
  const userTags = await getTags(uid);
  const userTagIds = userTags.map((tag) => tag._id.toString());
  let validTags = true;
  tags.forEach((tagId) => {
    if (!userTagIds.includes(tagId)) validTags = false;
  });
  if (!validTags) {
    throw new TypesterError(422, "One of the tag id's is not valid");
  }
  return await db
    .collection<TypesterResult>("results")
    .updateOne({ _id: new ObjectId(resultId), uid }, { $set: { tags } });
}

export async function getResult(
  uid: string,
  id: string
): Promise<TypesterResult> {
  const result = await db
    .collection<TypesterResult>("results")
    .findOne({ _id: new ObjectId(id), uid });
  if (!result) throw new TypesterError(404, "Result not found");
  return result;
}

export async function getLastResult(
  uid: string
): Promise<Partial<TypesterResult>> {
  const [lastResult] = await db
    .collection<TypesterResult>("results")
    .find({ uid })
    .sort({ timestamp: -1 })
    .limit(1)
    .toArray();
  if (!lastResult) throw new TypesterError(404, "No results found");
  return _.omit(lastResult, "uid");
}

export async function getResultByTimestamp(
  uid: string,
  timestamp
): Promise<TypesterResult | null> {
  return await db
    .collection<TypesterResult>("results")
    .findOne({ uid, timestamp });
}

export async function getResults(
  uid: string,
  start?: number,
  end?: number
): Promise<TypesterResult[]> {
  start = start ?? 0;
  end = end ?? 1000;
  const results = await db
    .collection<TypesterResult>("results")
    .find({ uid })
    .sort({ timestamp: -1 })
    .skip(start)
    .limit(end)
    .toArray(); // this needs to be changed to later take patreon into consideration
  if (!results) throw new TypesterError(404, "Result not found");
  return results;
}
